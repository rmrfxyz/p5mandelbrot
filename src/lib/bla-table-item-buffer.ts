...
// Most of this file was generated by ChatGPT
...
export function encodeBlaTableItems(items: BLATableItem[][]): ArrayBuffer {
  ...
  // Add the number of rows and the number of bytes needed to store the number of elements in each row.
  let totalSize = 4; // 最初の4バイトは行の数 The first 4 bytes are the number of lines
  items.forEach((row) => {
    totalSize += 4; // 各行の要素数を格納するための4バイト 4 bytes to store the number of elements in each row
    totalSize += row.length * ITEM_BYTE_LENGTH; // 実際の各行のデータ Actual data for each row
  });
  ...
  // 最初のエントリに行の数を設定
  // Set the number of rows in the first entry
  view[0] = items.length;
  let offset = 1; // Int32のエントリとしてのオフセット The offset as an Int32 entry

  items.forEach((row) => {
    view[offset] = row.length; // 各行の要素数を設定 Set the number of elements in each row
    offset += 1; // 次の要素数のためにオフセットを1つ進める Advance the offset by one for the next number of elements

    row.forEach((item) => {
      const itemBuffer = encodeBlaTableItem(item);
      // Int32のエントリではなく、バイトとしてのオフセットを計算する必要がある
      // We need to calculate the offset as bytes, not as an Int32 entry.
      const byteOffset = offset * 4;
      new Uint8Array(buffer, byteOffset, ITEM_BYTE_LENGTH).set(
        new Uint8Array(itemBuffer),
      );
      offset += ITEM_BYTE_LENGTH / 4; // 次のアイテムのためにオフセットをアイテムのバイト長分進める Advance the offset for the next item by the item's length in bytes
    });
  });
...
export function decodeBLATableItems(buffer: ArrayBuffer): BLATableItem[][] {
  ...
  const rows = view.getInt32(0, true); // 最初のエントリは行の数 The first entry is the number of lines
  ...
  for (let i = 0; i < rows; i++) {
    ...
    for (let j = 0; j < rowLength; j++) {
      ...
      // Calculate the offset as bytes, not as an Int32 entry
      const byteOffset = offset * 4;
